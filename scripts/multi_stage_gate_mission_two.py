...
from navigation.missions import MultiStageGateMissionLevelTwo
...
if __name__ == "__main__":
    args = parse_args()
    # Must target navigation.navigation (where hitbox_fits_gate reads these),
    # not the navigation package, or the overrides are silently lost.
    from navigation import navigation as _navmod
    size_m = args.hitbox_size_inches * 0.0254
    margin_m = args.hitbox_margin_inches * 0.0254
    _navmod.DRONE_HITBOX_M = (size_m, size_m, size_m)
    _navmod.HITBOX_MARGIN_M = margin_m

    nav = NavigationController(MAVLINK_CONN)
    mission = MultiStageGateMissionLevelTwo(
    ...
    try:
        print("[*] Starting Level 2 Multi-Stage Gate Mission")
