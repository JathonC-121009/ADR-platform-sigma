...
## 📦 Included Files
Ensure your project directory contains the following files before proceeding:
* `best.hef` *(Compiled hardware AI model)*
* `main.py` *(Application entry point)*
* `opencv_processing.py` *(Core AI pipeline, kinematics, and HUD)*
* `gstreamer_class.py` *(Hardware camera stream handler)*
* `camera_app.py` & `flask_streaming.py` *(Web server/streaming modules)*
* `debug_hailo.py` *(Hardware diagnostic tool)*
* `tevs_CAM1_fisheye_calibration.yaml` *(Calibration for Camera 1)*
* `tevs_CAM2_fisheye_calibration.yaml` *(Calibration for Camera 2)*
...
scp * <PI_USERNAME>@<PI_IP_ADDRESS>:~/drone_vision/
```
...
Before running, open `opencv_processing.py` and ensure the configuration toggles at the very top of the file match your setup:
