...
USE_APRILTAG_IDS = True
APRILTAG_DICT = cv2.aruco.DICT_APRILTAG_36h11
NO_TAG_ID = -1
STICKY_MAX_MISS = 15    # frames a tag can vanish before index 0 is released
# ==========================================
...
class OpenCVProcessing():
    def __init__(self, cam: CameraStream):
        ...
        # --- SETUP APRILTAG DETECTOR (built once, not per frame) ---
        ...
        if USE_APRILTAG_IDS:
            ...
            self.tag_detector = cv2.aruco.ArucoDetector(
                cv2.aruco.getPredefinedDictionary(APRILTAG_DICT), params)
            print("[INFO] AprilTag (36h11) ID detection enabled.")
    ...
    def detect_tags(self, clean_frame):
        """Return (centers, corners, ids). Reads an undrawn frame, draws nothing."""
        if self.tag_detector is None:
            return [], None, None

        gray = cv2.cvtColor(clean_frame, cv2.COLOR_BGR2GRAY)
        corners, ids, _ = self.tag_detector.detectMarkers(gray)
        if ids is None:
            return [], None, None

        centers = [(c.reshape(4, 2).mean(axis=0), int(i))
                   for c, i in zip(corners, ids.flatten())]
        return centers, corners, ids

    def cv_processing_loop(self):
        prev_tracked_gates = []
        sticky_id, sticky_miss = NO_TAG_ID, 0
        ALPHA = 0.3
        ...
        while True:
            ...
            # `clean` is the analysis buffer, `frame` is the display buffer.
            # Everything below draws on `frame`, so crops must come from `clean`
            # or one gate's annotations corrupt the next gate's orange mask.
            clean = frame.copy()
            tag_centers, tag_corners, tag_ids = self.detect_tags(clean)
                    ...
                    if crop_corners is not None:
                        ...
                        if not is_partial:
                            ...
                            if success:
                                ...
                                # --- CLAIM AN APRILTAG ID FOR THIS GATE ---
                                tag_id = NO_TAG_ID
                                for center, tid in tag_centers:
                                    if x1 <= center[0] <= x2 and y1 <= center[1] <= y2:
                                        tag_id = tid
                                        break

                                # Same tag = same gate, exactly. Fall back to the
                                # position-jump test only when the tag is unknown.
                                best_match = None
                                if tag_id != NO_TAG_ID:
                                    best_match = next(
                                        (p for p in prev_tracked_gates
                                         if p['tag_id'] == tag_id), None)

                                if best_match is None:
                                    min_dist = float('inf')
                                    for ptg in prev_tracked_gates:
                                        if tag_id != NO_TAG_ID and ptg['tag_id'] != NO_TAG_ID:
                                            continue  # both tagged, different tags
                                ...
                                if tag_id != NO_TAG_ID:
                                    cv2.putText(frame, f"TAG {tag_id}", (x1, y1 - 10),
                                                cv2.FONT_HERSHEY_SIMPLEX, 0.8, (0, 255, 255), 2)

            # Safe to annotate now that every crop has been taken.
            if tag_ids is not None:
                cv2.aruco.drawDetectedMarkers(frame, tag_corners, tag_ids)
            ...
            # Keep the same tag at index 0 across frames. The nav side only reads
            # gates[0], so letting it flicker between two similar-range gates
            # corrupts the averaging window in observe_gate().
            if sticky_id != NO_TAG_ID:
                idx = next((k for k, g in enumerate(current_tracked_gates)
                            if g['tag_id'] == sticky_id), None)
                if idx is None:
                    sticky_miss += 1
                    if sticky_miss > STICKY_MAX_MISS:
                        sticky_id = NO_TAG_ID
                else:
                    sticky_miss = 0
                    current_tracked_gates.insert(0, current_tracked_gates.pop(idx))

            if sticky_id == NO_TAG_ID and current_tracked_gates:
                # May latch NO_TAG_ID for an untagged gate, which simply leaves
                # the original nearest-first behaviour in place.
                sticky_id = current_tracked_gates[0]['tag_id']
            ...
            while len(telemetry_matrix) < 3:
                telemetry_matrix.append([999.0, 999.0, 999.0, 999.0, 999.0, 999.0, 999.0, NO_TAG_ID])

            # --- VISUALIZE TELEMETRY MATRIX ON SCREEN ---
            y_offset = 70
            cv2.putText(frame, "Telemetry Matrix [Dist, Fwd(X), Rgt(Y), Dwn(Z), Roll, Pitch, Yaw, ID]:",
            ...
            try:
                ...
                # Never silent: a serialization bug here blinds the drone completely.
                print(f"[ERROR] Telemetry serialization failed: {e}")
            # =========================================================
